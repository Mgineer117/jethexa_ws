#!/usr/bin/env python3
"""
Live closed-loop controller for the JetHexa.

Loads a trained policy network (PPO, CARL, or C3M) from models/, replays a
reference trajectory from models/test_traj.npz, and at every control tick
asks the policy for a corrective joint command on top of the reference.
The resulting absolute 18-DOF joint targets are published to the robot,
and the full rollout (states, actions) is saved back to hexapod_data/ for
later analysis.

Sensor configuration sets `use_mocap=True`, so qualysis.py must already be
publishing to /qualysis/<deck> before this script is launched, or it will
block forever waiting for the pose stream.

Example:
    python scripts/control_loop.py --algo-name ppo --control-scaler 0.3 --duration 33
"""
...
# Resolve repo root and chdir there so "models/*.pth", "hexapod_data/", and
# `from models.policy_networks import ...` all work after the move into scripts/.
