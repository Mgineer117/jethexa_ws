    ...
    def __init__(self, directory=".", dt=0.1, max_files=None):
        ...
        for f in files:
            ...
            try:
                ...
                # Construct dynamics using finite difference: \dot{x} = (x_{t+1} - x_t) / dt
                x = states_wrapped[:-1, :]

                x_unwrapped = states_unwrapped[:-1, :]
                x_next_unwrapped = states_unwrapped[1:, :]
                x_dot = (x_next_unwrapped - x_unwrapped) / self.dt
...
class DataDrivenDynamics(nn.Module):
    def __init__(self, state_dim=23, base_dim=5, u_dim=18):
