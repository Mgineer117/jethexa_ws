...
class jethexaControlNode:
    def __init__(self, node_name):
        ...
        self.set_transform2_sub = rospy.Subscriber(
            "~pose_transform_euler", jetmsg.TransformEuler, self.pose_transform_euler_cb
        )
        ...
        self.set_pose_euler_sub = rospy.Subscriber(
            "~set_pose_euler", jetmsg.Pose, self.set_pose_euler_cb
        )
        ...
        self.set_leg_position_sub = rospy.Subscriber(
            "~set_leg_absolute", jetmsg.LegPosition, self.set_leg_absolute_cb
        )
        ...
        self.set_leg_position_re_sub = rospy.Subscriber(
            "~set_leg_relatively", jetmsg.LegPosition, self.set_leg_relatively_cb
        )
        ...
        self.set_head_absolute = rospy.Subscriber(
            "~set_head_absolute", jetmsg.TransformEuler, self.head_absolute_cb
        )
        # relative rotation
        self.set_head_relatively = rospy.Subscriber(
            "~set_head_relatively", jetmsg.TransformEuler, self.head_relatively_cb
        )

        # robot movement service
        # control the robot to move through gait parameter
        self.traveling = rospy.Subscriber(
            "~traveling", jetmsg.Traveling, self.set_traveling_cb
        )
        ...
        self.cmd_vel_sub = rospy.Subscriber(
            "~cmd_vel", geometry_msgs.msg.Twist, self.controller.cmd_vel
        )
        self.cmd_vel_sub_ = rospy.Subscriber(
            "cmd_vel", geometry_msgs.msg.Twist, self.controller.cmd_vel
        )

        # action group running service
        self.run_action_sub = rospy.Subscriber(
            "~run_actionset", jetmsg.RunActionSet, self.run_action_set_sub_cb
        )
